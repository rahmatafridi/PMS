...
const state = documentInitialState()
...
const actions = {
  fetchList: async ({ state, commit }, { auth }) => {
    try {
      ...
      let req = await DocumentApi.document.list({
...
const mutations = {
  ...
  setListDocument(state, { note, value }) {
  ...
  RESET(state) {
    const newState = documentInitialState();
